"""
GLPI Agent Task Inventory MacOS Sound Module

This module collects sound device information on macOS systems.
"""
...
class Sound:
    ...
    def do_inventory(self, **params):
        """
        Perform the sound device inventory and add results to inventory.
        ...
        infos = self._get_system_profiler_infos(
            type='SPAudioDataType',
            logger=logger
        )
        
        info = infos.get('Audio (Built In)', {})
        
        for sound in sorted(info.keys()):
            inventory.add_entry(
                section='SOUNDS',
                entry={
                    'NAME': sound,
