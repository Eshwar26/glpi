"""
GLPI Agent Task Deploy CheckProcessor WinValueExists Module
"""

import platform


class WinValueExists:
    """Check if Windows registry value exists"""
    
    def __init__(self, path, value=None, logger=None):
        """Initialize Windows value exists check"""
        self.path = path
        self.value = value
        self.logger = logger
    ...
    def prepare(self) -> None:
        ...
        self._on_success = f"registry value found: {self.path}"
    
    def success(self) -> bool:
        """Check if registry value exists (Windows only)"""
