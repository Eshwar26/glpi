#!/usr/bin/env python3
"""PerlBuildJob - Perl build job configuration for Windows packaging"""
...
def build_job(arch: str, rev: str, notest: bool, dllsuffix: str) -> Dict[str, Any]:
    """
    Get build job configuration for GLPI Agent.
    
    Args:
        arch: Architecture ('x86' or 'x64')
        rev: Revision number
        notest: Skip tests flag
        dllsuffix: DLL suffix for architecture
    ...
    version_match = re.match(r'^(\d+)\.(\d+)\.', PERL_VERSION)
    MAJOR = version_match.group(1) if version_match else "5"
    MINOR = version_match.group(2) if version_match else "42"
    
    return {
        'app_version': f"{PERL_VERSION}.{rev}",  # BEWARE: do not use '.0.0' in the last two version digits
        'bits': 64 if arch == 'x64' else 32,
        'app_fullname': f"Strawberry Perl{' (64-bit)' if arch == 'x64' else ''}",
        ...
        'build_job_steps': [
            ...
            # STEP 1: Binaries cleanup
            {
                ...
                'commands': [
                    ...
                    {'do': 'removefile_recursive', 'args': ['<image_dir>/c', r'.+\.la$']},  # https://rt.cpan.org/Public/Bug/Display.html?id=127184
            ...
            # STEP 2: Build perl
            {
                ...
                'url': f'https://www.cpan.org/src/5.0/perl-{PERL_VERSION}.tar.gz',
            ...
            # STEP 3: Sign perl DLL
            {
                'plugin': 'CustomCodeSigning',
                'files': [
                    f'<image_dir>/perl/bin/perl{MAJOR}{MINOR}.dll',
                ],
            },
            # STEP 4: Upgrade CPAN modules
            {
                'plugin': 'Perl::Dist::Strawberry::Step::UpgradeCpanModules',
            },
            # STEP 5: Install needed modules with agent dependencies
            {
                ...
                'modules': [
                    ...
                    'IPC-Run',
                    # win32 related
                    'Win32::API', 'Win32API::Registry', 'Win32::TieRegistry', 'Win32::OLE',
                    'Win32-Daemon', 'Win32::Job', 'Sys::Syslog',
                    # file related
                    'File::Copy::Recursive', 'File::Which',
                    # SSL
                    'Net-SSLeay', 'Mozilla::CA', 'IO-Socket-SSL',
                    # network
                    ...
                    'IO::Socket::IP', 'IO::Socket::INET6', 'HTTP::Daemon',
                    'HTTP-Server-Simple', 'LWP::Protocol::https', 'LWP::UserAgent',
                    # crypto
                    {'module': 'https://github.com/g-bougard/Crypt-DES/releases/download/2.07_01/Crypt-DES-2.07_01.tar.gz'},  # Patched Crypt::DES
                    'Crypt::Rijndael',
                    'Digest-SHA',
                    'Digest-MD5', 'Digest-SHA1', 'Digest::HMAC',  # Required for SNMP v3 authentication
                    # date/time
                    'DateTime', 'DateTime::TimeZone::Local::Win32',
                    # GLPI-Agent deps
                    'Text::Template', 'UNIVERSAL::require', 'UNIVERSAL::isa', 'Net::SSH2',
            ...
            # STEP 7: Clean up
            {
                ...
                'commands': [
                    ...
                    {'do': 'removefile_recursive', 'args': ['<image_dir>/perl', r'.+\.dll\.AA[A-Z]$']},
                    ...
                    {'do': 'removefile_recursive', 'args': ['<image_dir>/perl', r'^\.packlist$']},
                    {'do': 'removefile_recursive', 'args': ['<image_dir>/perl', r'\.pod$']},
                ],
            },
            # STEP 8: Install modules for test
            {
                'plugin': 'Perl::Dist::GLPI::Agent::Step::InstallModules',
                'modules': [
                    {
            ...
            # STEP 10: Clean up and finalize perl environment
            {
                'plugin': 'Perl::Dist::Strawberry::Step::FilesAndDirs',
                'commands': [
                    {'do': 'createdir', 'args': ['<image_dir>/perl/newbin']},
                    _movebin(f"libgcc_s_{'seh' if arch == 'x64' else 'dw2'}-1.dll"),
                    _movebin('libstdc++-6.dll'),
                    _movebin('libwinpthread-1.dll'),
                    _movebin('perl.exe'),
                    _movebin(f'perl{MAJOR}{MINOR}.dll'),
                    ...
                    {'do': 'removefile_recursive', 'args': ['<image_dir>/perl', r'\.a$']},
                    {'do': 'removedir', 'args': ['<image_dir>/bin']},
                    {'do': 'removedir', 'args': ['<image_dir>/c']},
                    {'do': 'removedir', 'args': [f'<image_dir>/{"x86_64" if arch == "x64" else "i686"}-w64-mingw32']},
                    ...
                    {'do': 'copyfile', 'args': [f'contrib/windows/packaging/tools/{arch}/7z.exe', '<image_dir>/perl/bin']},
                    {'do': 'copyfile', 'args': [f'contrib/windows/packaging/tools/{arch}/7z.dll', '<image_dir>/perl/bin']},
                ],
            },
            # STEP 11: Installation with direct github download
            {
                ...
                'downloads': [
                    ...
                    },
                    {
                        ...
                        'file': f'GLPI-AgentMonitor-{arch}.exe',
                        'folder': '<image_dir>/perl/bin',
                    },
                ],
            },
            # STEP 12: Run GLPI Agent test suite
            {
...
def _movebin(bin_name: str) -> Dict[str, Any]:
    """
    Create command to move binary file.
    
    Args:
        bin_name: Binary file name
    ...
    return {
        'do': 'movefile',
        'args': [
            f'<image_dir>/perl/bin/{bin_name}',
            f'<image_dir>/perl/newbin/{bin_name}'
        ]
    }


def _movedll(dll_base: str, suffix: str) -> Dict[str, Any]:
    """
    Create command to move DLL file.
    
    Args:
        dll_base: DLL base name (without suffix and extension)
        suffix: DLL suffix
        
    Returns:
        Command dictionary
    """
    file = f"{dll_base}{suffix}.dll"
    return {
        'do': 'movefile',
        'args': [
            f'<image_dir>/c/bin/{file}',
            f'<image_dir>/perl/newbin/{file}'
        ]
    }


__all__ = ['build_job', 'PERL_VERSION', 'PERL_BUILD_STEPS']
